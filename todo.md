...
- [x] Adicionar campo de referência rápida (somente leitura) no dossiê do efetivo apontando pro substituto atual — feito e em produção (commit 4b7ed22, 09/09/2026): `redaCadastros`/`redaEfetivosCobertos` incluídos em `listFunctionalData` (`server/db.ts`), lookup `redaSubstitutoAtualByEfetivoId` calculado no `Home.tsx` e exibido no dossiê logo após "Unidade / departamento".
- [x] Implementar alerta de teto de permanência (72 meses) para REDA (Docente e Técnico) — função `calcularAlertaTetoReda` no `Home.tsx` calcula automaticamente o marco de 36 meses (fim do contrato inicial, ponto de decisão de renovação) e o marco de 72 meses (teto final — usa `tetoPermanenciaData` se preenchido manualmente, senão `dataInicioContrato + 72 meses`), com selo de alerta a partir de 6 meses antes de cada marco. Exibido no card "Substituto REDA atual" do dossiê do efetivo. Falta levar o mesmo alerta pra tela própria do REDA quando o formulário de cadastro (item seguinte) for construído.
- [x] Construir a tela/formulário de cadastro REDA em si — botão "Registrar/Editar cadastro REDA" no dossiê do servidor, abrindo modal com todos os campos de `redaCadastros` (tipo, base legal, edital, datas de contrato, teto de permanência, status etc). Usa as mutations `createRedaCadastro`/`updateRedaCadastro` já existentes. Vinculo com o efetivo substituido (motivo do afastamento, portaria, numero do processo, data de inicio) e encerramento do vinculo (data de fim) tambem concluidos, usando as mutations `createRedaEfetivoCoberto`/`encerrarRedaEfetivoCoberto`. Modulo REDA completo: cadastro + vinculo + alerta de teto de permanencia (72 meses).
